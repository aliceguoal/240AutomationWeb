function refreshModule(name){
	...
	$.getJSON('ModuleConsumptionServlet?module=' + name + '&range=' + start + " to " + end, function(data){
	    ...
	    function buildConsumptionRow(){
			var pieData = data.toolgroups.map(function(d){
				return {name: d.toolgroup.name, value: d.total_consumption}
			});
			...
			piesvg.selectAll("path").data(pie(pieData)).enter()
				...
				.on("mouseover",function(d){
					var toolgroup = null;
					data.toolgroups.forEach(function(t){
						if(t.toolgroup.name == d.data.name)
							toolgroup = t;
					})
					var tempData = barData.map(function(v){
						return {name:v.name, value: toolgroup.mes_consumption[v.name].consumption}});
			...
			bars.append("rect").attr("class", "bar")
			    ...
			    .on("mouseover", function(d){
			    	var tempData = data.toolgroups.map(function(t){
			    		if(t.mes_consumption[d.name] == null)
			    			return {name: t.toolgroup.name, value: 0};
		    			else
		    				return {name: t.toolgroup.name, value: t.mes_consumption[d.name].consumption};
					});
			    	updatePie(tempData);
	    ...
	    function buildWasteRow(){
			var pieData = data.toolgroups.map(function(d){
				return {name: d.toolgroup.name, value: d.total_waste}
			});
			...
			piesvg.selectAll("path").data(pie(pieData)).enter()
				...
				.on("mouseover",function(d){
					...
					var tempData = barData.map(function(v){
						return {name:v.name, value: toolgroup.mes_consumption[v.name].waste}});
					updateBar(tempData, color(d.data.name));
				})
				.on("mouseout", function(d){
					updateBar(barData, 'steelblue');
			    ...
			    .on("mouseover", function(d){
			    	var tempData = data.toolgroups.map(function(t){
			    		if(t.mes_consumption[d.name] == null)
		    				...
		    				return {name: t.toolgroup.name, value: t.mes_consumption[d.name].waste};
					});
			    	updatePie(tempData);
			    	d3.select(this).style("fill", "brown");
			    })
